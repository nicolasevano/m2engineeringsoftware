...
System.out.println("initTete : EOF :"+ bande.finDeFichier()+" Fini :"+fini());
		

		// if (!bande.finDeFichier() && ! fini()) {
			if (!bande.finDeFichier() ) {	
			tete = bande.lireUnCar();
			System.out.println(" InitTete EOF2:"+ bande.finDeFichier()+" Fini2:"+fini());
			// System.out.print((char)tete); // pb pour sortie le code source !!!!
		} else {tete = -10;}
		
		System.out.println("InitTete tete"+tete);
	}
	/**
	...
	public void forcerTete () {
		if (!bande.finDeFichier()) {tete = bande.lireUnCar();} else {tete = -15;}
	}
	/**
	 ...
	 * Pose une transition pour un intervalle [c1..c2] de caracteres
	 * @param P : etat source
	 * @param c1 : premire car 1 interval
	 * @param c2 : deuxieme car 1 interval
	 * @param c3 : premire car 1 interval
	 * @param c4 : deuxieme car 1 interval
	 * @param Q : etat cible
	 */
	protected void arcsPourUnionIntervalleT(int P, char c1, char c2, char c3, char c4, int Q){
		for(int i =(int)c1; i<=(int)c2;i++){
			unArcT(P,(char)i,Q);
		}
		for(int i =(int)c3; i<=(int)c4;i++){
			unArcT(P,(char)i,Q);
		}
	 ...
	 * @param A : action
	 */
	protected void arcsPourUnionIntervalleA(int P, char c1, char c2, char c3, char c4, int A){
		for(int i =(int)c1; i<=(int)c2;i++){
			unArcA(P,(char)i,A);
		}
		for(int i =(int)c3; i<=(int)c4;i++){
	 ...
	 * @param A : action
	 */
	protected void arcPourUnionIntervalle(int P, char c1, char c2, char c3, char c4, int Q,int A){
		arcsPourUnionIntervalleT(P,c1,c2,c3,c4,Q);
		arcsPourUnionIntervalleA(P,c1,c2,c3,c4,A);
	}
	 ...
	 * @param Q : etat cible
	 */
	protected void arcsPourUnionLettresT(int P, int Q){
		arcsPourUnionIntervalleT(P,'A','Z','a','z',Q);
	}
	 ...
	 * @param A : action
	 */
	protected void arcsPourUnionLettresA(int P, int A){
		arcsPourUnionIntervalleA(P,'A','Z','a','z',A);
	}
	 ...
	 * @param A : action
	 */
	protected void arcsPourUnionLettres(int P, int Q, int A){
		arcsPourUnionLettresT(P, Q);
		arcsPourUnionLettresA(P, A);
	}
	 ...
	 * 
	 * @param Q etat cible
	 * pose une transition P->Q pour tous les Ètats et tous les caracteres
	 */
	protected void arcs_defauts(int Q){
		for (int P=1;P<=NbEtats;P++)
		{  
			arcsPourIntervalleT(P, (char) c_code_min, (char) c_code_max, Q);
		}
	}
	/**
	 * creer_tables_vides
...
System.out.println("Etat :"+i+"->"+(char)j+"->"+table_transitions[i] [j]+" ("+table_actions[i][j]+")");
	...
	public void lancer (){ 
		...
		while (!bande.finDeFichier() && ! fini()) {
			...
			System.out.print("Etat courant :"+etatCourant+" "+tete);
